...
if (0) {
  # Take data frame produced by AdSpaceSummary.R
  twdf <- rel_october_red
  final_ad_names <- rel_ads_name
  l_ads <- length(final_ad_names)
  final_adspace_names <- rel_adspace_name
...
# Remove zero columns = useless ads (rows=spaces are ok)
if (1) { 
  zero_index = rep(TRUE,l_ads)
  for (j in 1:l_ads) {if (sum(index_matrix[,j])==0) { zero_index[j] = FALSE }}
  #zero_index = zero_index[-1]
  index_matrix_red <- index_matrix[,zero_index]
  ctr_matrix_red <- ctr_matrix[,zero_index]  
  final_ad_names_red <- final_ad_names[zero_index]
}
# write matrices to files
if (1) {
  write.matrix(index_matrix_red,"index.matrix")
  write.matrix(ctr_matrix_red,"recom.matrix")
}

# plot population/occupation pattern of index matrix
# Gebrauchtwagen-Werbung: Rows 80-82
if (1) {
  
  library(reshape2)
  library(ggplot2)
  #m = matrix(rnorm(20),5)
  ggplot(melt(t(index_matrix_red)), aes(Var1,Var2, fill=value))  + geom_raster()
...
# write data to files
write.table(final_adspace_names,file="adspace_ids.txt",sep=" ",col.names=FALSE)
